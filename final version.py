...
def start_game(event):
    global game_started, start_time  # Declare start_time as a global variable
    game_started = True
    start_time = time.time()
    ...
    if game_started:
        ...
        if time_left == 0:
            ...
            while True:
                ...
                cv2.putText(frame, "Game Over", (WIDTH // 2 - 80, HEIGHT // 2 - 20), cv2.FONT_HERSHEY_SIMPLEX, 1, RED, 2)

                # Display the player's score
                cv2.putText(frame, f"Score: {score}", (WIDTH // 2 - 45, HEIGHT // 2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 1, RED, 2)

                # Display quit instructions
                cv2.putText(frame, "Press 'q' to quit", (WIDTH // 2 - 60, HEIGHT // 2 + 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, RED, 2)
